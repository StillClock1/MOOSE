...
RANGE.Sound = {
    ...
    RCAccuracy = {
        filename = "RC-Accuracy.ogg",
        duration = 0.67
    },
    RCDegrees = {
        filename = "RC-Degrees.ogg",
        duration = 0.59
    },
    RCExcellentHit = {
        filename = "RC-ExcellentHit.ogg",
        duration = 0.76
    },
    RCExcellentPass = {
        filename = "RC-ExcellentPass.ogg",
        duration = 0.89
    },
    RCFeet = {
        filename = "RC-Feet.ogg",
        duration = 0.49
    },
    RCFor = {
        filename = "RC-For.ogg",
    ...
    RCGoodHit = {
        filename = "RC-GoodHit.ogg",
        duration = 0.52
    },
    RCGoodPass = {
        filename = "RC-GoodPass.ogg",
        duration = 0.62
    },
    RCHitsOnTarget = {
        filename = "RC-HitsOnTarget.ogg",
        duration = 0.88
    },
    RCImpact = {
        filename = "RC-Impact.ogg",
        duration = 0.61
    },
    RCIneffectiveHit = {
        filename = "RC-IneffectiveHit.ogg",
        duration = 0.86
    },
    RCIneffectivePass = {
        filename = "RC-IneffectivePass.ogg",
        duration = 0.99
    },
    RCInvalidHit = {
        filename = "RC-InvalidHit.ogg",
        duration = 2.97
    },
    RCLeftStrafePitTooQuickly = {
        filename = "RC-LeftStrafePitTooQuickly.ogg",
        duration = 3.09
    },
    RCPercent = {
        filename = "RC-Percent.ogg",
        duration = 0.56
    },
    RCPoorHit = {
        filename = "RC-PoorHit.ogg",
        duration = 0.54
    },
    RCPoorPass = {
        filename = "RC-PoorPass.ogg",
        duration = 0.68
    },
    RCRollingInOnStrafeTarget = {
        filename = "RC-RollingInOnStrafeTarget.ogg",
        duration = 1.38
    },
    RCTotalRoundsFired = {
        filename = "RC-TotalRoundsFired.ogg",
        duration = 1.22
    },
    RCWeaponImpactedTooFar = {
        filename = "RC-WeaponImpactedTooFar.ogg",
        duration = 3.73
    },
    IR0 = {
        filename = "IR-0.ogg",
    ...
    IRDecimal = {
        filename = "IR-Decimal.ogg",
        duration = 0.54
    },
    IRMegaHertz = {
        filename = "IR-MegaHertz.ogg",
        duration = 0.87
    },
    IREnterRange = {
        filename = "IR-EnterRange.ogg",
        duration = 4.83
    },
    IRExitRange = {
        filename = "IR-ExitRange.ogg",
        duration = 3.10
    }
}
...
function RANGE:onafterStart()
    ...
    if self.location == nil then
        local text = string.format(
            "ERROR! No range location found. Number of strafe targets = %d. Number of bomb targets = %d.",
            self.nstrafetargets, self.nbombtargets)
    ...
    if self.rangezone == nil then
        self.rangezone = ZONE_RADIUS:New(self.rangename, {
            x = self.location.x,
            y = self.location.z
        }, self.rangeradius)
    end
    ...
    -- Starting range.
    local text = string.format("Starting RANGE %s. Number of strafe targets = %d. Number of bomb targets = %d.",
        self.rangename, self.nstrafetargets, self.nbombtargets)
    self:I(self.lid .. text)
...
function RANGE:AddStrafePit(targetnames, boxlength, boxwidth, heading, inverseheading, goodpass, foulline)
    ...
    if ntargets == 0 then
        local text = string.format(
            "ERROR! No strafe target could be found when calling RANGE:AddStrafePit() for range %s", self.rangename)
        self:E(self.lid .. text)
        return
    end
    ...
    -- Debug info
    local text = string.format(
        "Adding new strafe target %s with %d targets: heading = %03d, box_L = %.1f, box_W = %.1f, goodpass = %d, foul line = %.1f",
        _name, ntargets, heading, l, w, goodpass, foulline)
...
function RANGE:AddBombingTargets(targetnames, goodhitrange, randommove)
    ...
    for _, name in pairs(targetnames) do
        ...
        if _isstatic == true then
            ...
            self:T2(self.lid ..
                        string.format("Adding static bombing target %s with hit range %d.", name, goodhitrange, false))
            self:AddBombingTargetUnit(_static, goodhitrange)
        elseif _isstatic == false then
            local _unit = UNIT:FindByName(name)
            self:T2(self.lid ..
                        string.format("Adding unit bombing target %s with hit range %d.", name, goodhitrange, randommove))
...
function RANGE:AddBombingTargetUnit(unit, goodhitrange, randommove)
    ...
    if _isstatic == true then
        self:T(self.lid ..
                   string.format("Adding STATIC bombing target %s with good hit range %d. Random move = %s.", name,
                goodhitrange, tostring(randommove)))
    elseif _isstatic == false then
        self:T(self.lid ..
                   string.format("Adding UNIT bombing target %s with good hit range %d. Random move = %s.", name,
                goodhitrange, tostring(randommove)))
    else
        self:E(self.lid .. string.format(
            "ERROR! No bombing target with name %s could be found. Carefully check all UNIT and STATIC names defined in the mission editor!",
            name))
    end
...
function RANGE:GetFoullineDistance(namepit, namefoulline)
    ...
    elseif _staticpit == false then
        ...
        self:E(self.lid ..
                   string.format(
                "ERROR! Pit object %s could not be found in GetFoullineDistance function. Check the name in the ME.",
                namepit))
    end
    ...
    elseif _staticfoul == false then
        ...
        self:E(self.lid .. string.format(
            "ERROR! Foul line object %s could not be found in GetFoullineDistance function. Check the name in the ME.",
            namefoulline))
    end
    ...
    if pit ~= nil and foul ~= nil then
        ...
        self:E(self.lid .. string.format(
            "ERROR! Foul line distance could not be determined. Check pit object name %s and foul line object name %s in the ME.",
            namepit, namefoulline))
    end
...
function RANGE:OnEventBirth(EventData)
    self:F({
        eventbirth = EventData
    })

    if not EventData.IniPlayerName then
        return
    end
    ...
    if _unit and _playername then
        ...
        -- Debug output.
        local text = string.format("Player %s, callsign %s entered unit %s (UID %d) of group %s (GID %d)", _playername,
            _callsign, _unitName, _uid, _group:GetName(), _gid)
        self:T(self.lid .. text)
        ...
        self.PlayerSettings[_playername].smokeDuration = 30
            ...
            if _target and _target:IsAlive() and _target:GetName() == targetname then
                ...
                if dist > _currentTarget.zone.foulline then
                    ...
                    if _currentTarget.pastfoulline == false and _unit and _playername then
                        ...
                        -- DONE - SRS output
                        local text = string.format(
                            "%s, Invalid hit!\nYou already passed foul line distance of %d m for target %s.",
                            self:_myname(_unitName), _d, targetname)
                        if self.useSRS then
                            local ttstext = string.format(
                                "%s, Invalid hit! You already passed foul line distance of %d meters for target %s.",
                                self:_myname(_unitName), _d, targetname)
...
--- Internal helper function to stop smoke at a coordinate.
-- @param Core.Point#COORDINATE coordOfSmoke The coordinate where smoke should be stopped.
-- @param #string name The unique smoke name to stop.
local function RemoveSmokeTimed(coordOfSmoke, name)
    env.info("  ------Removing smoke: " .. (name or "nil"))
    -- coordOfSmoke:MarkToAll("SMOKE STOPPING")
    coordOfSmoke:StopSmoke(name)
end

--- Creates smoke at a coordinate in the specified color, and automatically stops it after 30 seconds.
-- @param Core.Point#COORDINATE CoordToSmoke The coordinate where the smoke should be placed.
-- @param #number colour The SMOKECOLOR enum (e.g., SMOKECOLOR.Red, SMOKECOLOR.White).
-- @param #string name (Optional) A unique name to track and stop this smoke.
local function SmokeCoordinate(CoordToSmoke, colour, name)
    env.info("----STARTING SMOKE AT " .. name)

    if not CoordToSmoke or not colour then
        env.info("  [SmokeCoordinate] Invalid coordinate or color.")
        return
    end

    CoordToSmoke:Smoke(colour, name)

    env.info("  -----SMOKE TIMER START 30s------")

    local timerSmoke = TIMER:New(RemoveSmokeTimed, CoordToSmoke, name)
    timerSmoke:Start(self.PlayerSettings[_playername].smokeDuration)
end
...
function RANGE._OnImpact(weapon, self, playerData, attackHdg, attackAlt, attackVel)
    ...
    if playerData and playerData.smokebombimpact and insidezone then
        env.info("--------playerData and playerData.smokebombimpact and insidezone-----------")

        -- Generate a unique name for the smoke
        local smokename = string.format("impactsmoke_%s_%d", playerData.playername or "anon", timer.getTime())

        if playerData.delaysmoke then
            env.info("----SMOKE DELAY ACTIVE-----")
            timer.scheduleFunction(self._DelayedSmoke, {
                coord = impactcoord,
                color = playerData.smokecolor,
                name = smokename -- pass the name to the delayed function
            }, timer.getTime() + self.TdelaySmoke)
        else
            env.info("----NO SMOKE DELAY ACTIVE-----")
            SmokeCoordinate(impactcoord, playerData.smokecolor, smokename)
        end
    else
        env.info("--------NOT playerData and playerData.smokebombimpact and insidezone-----------")
    end
    ...
    if _distance and _distance <= self.scorebombdistance then
        ...
        if os then
            if os.date then
                env.info("os.date is available")
                env.info(os.date())
            else
                env.info("os available but os.date missing!")
            end
        else
            env.info("No os available")
        end
    ...
    elseif insidezone then
        ...
        -- Send message.
        -- DONE SRS message
        local _message = string.format("%s, weapon impacted too far from nearest range target (>%.1f km). No score!",
            _callsign, self.scorebombdistance / 1000)
        if self.useSRS then
            local ttstext = string.format(
                "%s, weapon impacted too far from nearest range target, mor than %.1f kilometer. No score!", _callsign,
                self.scorebombdistance / 1000)
            self.controlsrsQ:NewTransmission(ttstext, nil, self.controlmsrs, nil, 2)
        end
        ...
        if self.rangecontrol then
            ...
            if self.useSRS then
                ...
                self.rangecontrol:NewTransmission(self.Sound.RCWeaponImpactedTooFar.filename,
                    self.Sound.RCWeaponImpactedTooFar.duration, self.soundpath, nil, nil, _message, self.subduration)
            end
        end
...
function RANGE:OnEventShot(EventData)
    ...
    local _track = (weapon:IsBomb() and self.trackbombs) or (weapon:IsRocket() and self.trackrockets) or
                       (weapon:IsMissile() and self.trackmissiles)
    ...
    if _unit and _playername then
        ...
        self:T(self.lid ..
                   string.format("Range %s, player %s, player-range distance = %d km.", self.rangename, _playername,
                dPR / 1000))
    end
    ...
    if _track and dPR <= self.BombtrackThreshold and _unit and _playername and self.PlayerSettings[_playername] then
        ...
        self:T(self.lid ..
                   string.format("RANGE %s: Tracking %s - %s.", self.rangename, weapon:GetTypeName(), weapon:GetName()))
        ...
        self:T(self.lid ..
                   string.format("Range %s, player %s: Tracking of weapon starts in 0.1 seconds.", self.rangename,
                _playername))
...
function RANGE:onafterStatus(From, Event, To)
    ...
    if self.verbose > 0 then
        ...
        if self.instructor then
            ...
            text = text ..
                       string.format(", Instructor %.3f MHz (Relay=%s alive=%s)", self.instructorfreq,
                    tostring(self.instructorrelayname), alive)
        end
        ...
        if self.rangecontrol then
            ...
            text = text ..
                       string.format(", Control %.3f MHz (Relay=%s alive=%s)", self.rangecontrolfreq,
                    tostring(self.rangecontrolrelayname), alive)
        end
...
function RANGE:onafterEnterRange(From, Event, To, player)
    ...
    if self.instructor and self.rangecontrol then
        ...
        if self.useSRS then
            ...
            local text = string.format(
                "You entered the bombing range. For hit assessment, contact the range controller at %.3f MHz",
                self.rangecontrolfreq)
            local ttstext = string.format(
                "You entered the bombing range. For hit assessment, contact the range controller at %.3f mega hertz.",
                self.rangecontrolfreq)
            ...
            self.instructor:NewTransmission(self.Sound.IREnterRange.filename, self.Sound.IREnterRange.duration,
                self.soundpath)
            self.instructor:Number2Transmission(RF[1])

            if tonumber(RF[2]) > 0 then
                self.instructor:NewTransmission(self.Sound.IRDecimal.filename, self.Sound.IRDecimal.duration,
                    self.soundpath)
                self.instructor:Number2Transmission(RF[2])
            end

            self.instructor:NewTransmission(self.Sound.IRMegaHertz.filename, self.Sound.IRMegaHertz.duration,
                self.soundpath)
        end
    end
...
function RANGE:onafterExitRange(From, Event, To, player)
    ...
    if self.instructor then
        ...
        if self.useSRS then
            ...
            self.instructor:NewTransmission(self.Sound.IRExitRange.filename, self.Sound.IRExitRange.duration,
                self.soundpath)
        end
    end
...
function RANGE:onafterImpact(From, Event, To, result, player)
    ...
    local text = string.format("%s, impact %03d° for %d ft (%d m)", player.playername, result.radial,
        UTILS.MetersToFeet(result.distance), result.distance)
    if targetname then
    ...
    if self.rangecontrol then
        ...
        if self.useSRS then
            ...
            self.rangecontrol:NewTransmission(self.Sound.RCImpact.filename, self.Sound.RCImpact.duration,
                self.soundpath, nil, nil, text, self.subduration)
            self.rangecontrol:Number2Transmission(string.format("%03d", result.radial), nil, 0.1)
            self.rangecontrol:NewTransmission(self.Sound.RCDegrees.filename, self.Sound.RCDegrees.duration,
                self.soundpath)
            ...
            if result.quality == "POOR" then
                self.rangecontrol:NewTransmission(self.Sound.RCPoorHit.filename, self.Sound.RCPoorHit.duration,
                    self.soundpath, nil, 0.5)
            elseif result.quality == "INEFFECTIVE" then
                self.rangecontrol:NewTransmission(self.Sound.RCIneffectiveHit.filename,
                    self.Sound.RCIneffectiveHit.duration, self.soundpath, nil, 0.5)
            elseif result.quality == "GOOD" then
                self.rangecontrol:NewTransmission(self.Sound.RCGoodHit.filename, self.Sound.RCGoodHit.duration,
                    self.soundpath, nil, 0.5)
            elseif result.quality == "EXCELLENT" then
                self.rangecontrol:NewTransmission(self.Sound.RCExcellentHit.filename,
                    self.Sound.RCExcellentHit.duration, self.soundpath, nil, 0.5)
            end
        end
    end
...
function RANGE:onafterSave(From, Event, To)
    ...
    for playername, results in pairs(self.bombPlayerResults) do
        ...
        for i, _result in pairs(results) do
            ...
            scores = scores ..
                         string.format("\n%s,%d,%s,%.2f,%03d,%s,%s,%s,%s,%s", playername, i, target, distance, radial,
                    quality, weapon, airframe, time, date)
        end
    end
...
function RANGE:onafterLoad(From, Event, To)
    ...
    local function _loadfile(filename)
        ...
        if f then
            ...
            self:E(self.lid ..
                       string.format(
                    "WARNING: Could not load player results from file %s. File might not exist just yet.",
                    tostring(filename)))
            return nil
        end
    end
...
function RANGE:_SaveTargetSheet(_playername, result) -- RangeBoss Specific Function
    ...
    data = data ..
               string.format("%s,%s,%d,%d,%s,%s,%s,%s", _playername, target, roundsFired, roundsHit, strafeResult,
            airframe, time, date)
...
-- @param #table _args Arguments passed.
function RANGE._DelayedSmoke(_args)
    local coord = _args.coord
    local color = _args.color
    local name = _args.name or string.format("delayedsmoke_%d", timer.getTime())

    SmokeCoordinate(coord, color, name)
end
    ...
    if _unit and _playername then
        ...
        if _results == nil then
            ...
            for _, _result in pairs(_results) do
                ...
                -- Message text.
                _message = _message ..
                               string.format("\n[%d] Hits %d - %s - %s", _count, result.roundsHit, result.name,
                        result.roundsQuality)
...
function RANGE:_DisplayStrafePitResults(_unitName)
    ...
    if _unit and _playername then
        ...
        for _playerName, _results in pairs(self.strafePlayerResults) do
            ...
            if _best ~= nil then
                local text = string.format("%s: Hits %i - %s - %s", _playerName, _best.roundsHit, _best.name,
                    _best.roundsQuality)
                table.insert(_playerResults, {
                    msg = text,
                    hits = _best.roundsHit
                })
            end
    ...
    if _unit and _playername then
        ...
        if _results == nil then
            ...
            for i, _result in pairs(_results) do
                ...
                _message = _message .. "\n" ..
                               string.format("[%d] %d m %03d° - %s - %s - %s hit", i, result.distance, result.radial,
                        result.name, result.weapon, result.quality)

                -- Store best/first result.
                if _bestMsg == "" then
                    _bestMsg = string.format("%d m %03d° - %s - %s - %s hit", result.distance, result.radial,
                        result.name, result.weapon, result.quality)
                end
...
function RANGE:_DisplayBombingResults(_unitName)
    ...
    if _unit and _player then
        ...
        for _playerName, _results in pairs(self.bombPlayerResults) do
            ...
            if _best ~= nil then
                local bestres = string.format("%s: %d m - %s - %s - %s hit", _playerName, _best.distance, _best.name,
                    _best.weapon, _best.quality)
                table.insert(_playerResults, {
                    msg = bestres,
                    distance = _best.distance
                })
            end
        ...
        if _currentStrafeRun then -- player has already registered for a strafing run.
            ...
            if unitinzone then
                ...
                if _currentStrafeRun.time <= 3 then
                    ...
                    -- Message text.
                    local _msg = string.format("%s left strafing zone %s too quickly. No Score.", _playername,
                        _currentStrafeRun.zone.name)
                    ...
                    if self.rangecontrol then
                        if self.useSRS then
                            ...
                            self.rangecontrol:NewTransmission(self.Sound.RCLeftStrafePitTooQuickly.filename,
                                self.Sound.RCLeftStrafePitTooQuickly.duration, self.soundpath)
                        end
                    end
                else
                    ...
                    -- Message text.
                    local _text = string.format("%s, hits on target %s: %d", self:_myname(_unitName), _result.zone.name,
                        _result.hits)
                    local ttstext = string.format("%s, hits on target %s: %d.", self:_myname(_unitName),
                        _result.zone.name, _result.hits)
                    if shots and accur then
                        _text = _text .. string.format("\nTotal rounds fired %d. Accuracy %.1f %%.", shots, accur)
                        ttstext = ttstext ..
                                      string.format(". Total rounds fired %d. Accuracy %.1f percent.", shots, accur)
                    end
                    _text = _text .. string.format("\n%s", resulttext)
                    ...
                    if self.rangecontrol then
                        if self.useSRS then
                            ...
                            self.rangecontrol:NewTransmission(self.Sound.RCHitsOnTarget.filename,
                                self.Sound.RCHitsOnTarget.duration, self.soundpath)
                            self.rangecontrol:Number2Transmission(string.format("%d", _result.hits))
                            if shots and accur then
                                self.rangecontrol:NewTransmission(self.Sound.RCTotalRoundsFired.filename,
                                    self.Sound.RCTotalRoundsFired.duration, self.soundpath, nil, 0.2)
                                self.rangecontrol:Number2Transmission(string.format("%d", shots), nil, 0.2)
                                self.rangecontrol:NewTransmission(self.Sound.RCAccuracy.filename,
                                    self.Sound.RCAccuracy.duration, self.soundpath, nil, 0.2)
                                self.rangecontrol:Number2Transmission(string.format("%d", UTILS.Round(accur, 0)))
                                self.rangecontrol:NewTransmission(self.Sound.RCPercent.filename,
                                    self.Sound.RCPercent.duration, self.soundpath)
                            end
                ...
                if unitinzone then
                    ...
                    if self.rangecontrol then
                        if self.useSRS then
                            ...
                            self.rangecontrol:NewTransmission(self.Sound.RCRollingInOnStrafeTarget.filename,
                                self.Sound.RCRollingInOnStrafeTarget.duration, self.soundpath)
                        end
                    end
            ...
            if not self.MenuAddedTo[_gid] then
                ...
                elseif RANGE.MenuF10Root then
                    ...
                    if RANGE.MenuF10[_gid] == nil then
                        self:T2(self.lid ..
                                    string.format("Creating F10 menu 'On the Range' for group %s", group:GetName()))
                    else
                        self:T2(self.lid ..
                                    string.format("F10 menu 'On the Range' already EXISTS for group %s", group:GetName()))
                    end
                ...
                local _mysmokeDurationPath = MENU_GROUP:New(group, "Smoke Duration", _settingsPath)

                -- F10/On the Range/<Range Name>/Mark Targets/
                local _MoMap = MENU_GROUP_COMMAND:New(group, "Mark On Map", _markPath, self._MarkTargetsOnMap, self,
                    _unitName)
                local _IllRng = MENU_GROUP_COMMAND:New(group, "Illuminate Range", _markPath,
                    self._IlluminateBombTargets, self, _unitName)
                local _SSpit = MENU_GROUP_COMMAND:New(group, "Smoke Strafe Pits", _markPath,
                    self._SmokeStrafeTargetBoxes, self, _unitName)
                local _SStgts = MENU_GROUP_COMMAND:New(group, "Smoke Strafe Tgts", _markPath, self._SmokeStrafeTargets,
                    self, _unitName)
                local _SBtgts = MENU_GROUP_COMMAND:New(group, "Smoke Bomb Tgts", _markPath, self._SmokeBombTargets,
                    self, _unitName)
                -- F10/On the Range/<Range Name>/Stats/
                local _AllSR = MENU_GROUP_COMMAND:New(group, "All Strafe Results", _statsPath,
                    self._DisplayStrafePitResults, self, _unitName)
                local _AllBR = MENU_GROUP_COMMAND:New(group, "All Bombing Results", _statsPath,
                    self._DisplayBombingResults, self, _unitName)
                local _MySR = MENU_GROUP_COMMAND:New(group, "My Strafe Results", _statsPath,
                    self._DisplayMyStrafePitResults, self, _unitName)
                local _MyBR = MENU_GROUP_COMMAND:New(group, "My Bomb Results", _statsPath,
                    self._DisplayMyBombingResults, self, _unitName)
                local _ResetST = MENU_GROUP_COMMAND:New(group, "Reset All Stats", _statsPath, self._ResetRangeStats,
                    self, _unitName)
                -- F10/On the Range/<Range Name>/My Settings/Smoke Color/
                local _BlueSM = MENU_GROUP_COMMAND:New(group, "Blue Smoke", _mysmokePath, self._playersmokecolor, self,
                    _unitName, SMOKECOLOR.Blue)
                local _GrSM = MENU_GROUP_COMMAND:New(group, "Green Smoke", _mysmokePath, self._playersmokecolor, self,
                    _unitName, SMOKECOLOR.Green)
                local _OrSM = MENU_GROUP_COMMAND:New(group, "Orange Smoke", _mysmokePath, self._playersmokecolor, self,
                    _unitName, SMOKECOLOR.Orange)
                local _ReSM = MENU_GROUP_COMMAND:New(group, "Red Smoke", _mysmokePath, self._playersmokecolor, self,
                    _unitName, SMOKECOLOR.Red)
                local _WhSm = MENU_GROUP_COMMAND:New(group, "White Smoke", _mysmokePath, self._playersmokecolor, self,
                    _unitName, SMOKECOLOR.White)
                -- F10/On the Range/<Range Name>/My Settings/Flare Color/
                local _GrFl = MENU_GROUP_COMMAND:New(group, "Green Flares", _myflarePath, self._playerflarecolor, self,
                    _unitName, FLARECOLOR.Green)
                local _ReFl = MENU_GROUP_COMMAND:New(group, "Red Flares", _myflarePath, self._playerflarecolor, self,
                    _unitName, FLARECOLOR.Red)
                local _WhFl = MENU_GROUP_COMMAND:New(group, "White Flares", _myflarePath, self._playerflarecolor, self,
                    _unitName, FLARECOLOR.White)
                local _YeFl = MENU_GROUP_COMMAND:New(group, "Yellow Flares", _myflarePath, self._playerflarecolor, self,
                    _unitName, FLARECOLOR.Yellow)
                -- Add the smoke duration options to the menu
                local _SD30 = MENU_GROUP_COMMAND:New(group, "30 seconds", _mysmokeDurationPath,
                    self._playerSmokeDuration, self, _unitName, 30)
                local _SD60 = MENU_GROUP_COMMAND:New(group, "60 seconds", _mysmokeDurationPath,
                    self._playerSmokeDuration, self, _unitName, 60)
                local _SD90 = MENU_GROUP_COMMAND:New(group, "90 seconds", _mysmokeDurationPath,
                    self._playerSmokeDuration, self, _unitName, 90)
                local _SD120 = MENU_GROUP_COMMAND:New(group, "120 seconds", _mysmokeDurationPath,
                    self._playerSmokeDuration, self, _unitName, 120)
                local _SD300 = MENU_GROUP_COMMAND:New(group, "300 seconds", _mysmokeDurationPath,
                    self._playerSmokeDuration, self, _unitName, 300)

                -- F10/On the Range/<Range Name>/My Settings/
                local _SmDe = MENU_GROUP_COMMAND:New(group, "Smoke Delay On/Off", _settingsPath,
                    self._SmokeBombDelayOnOff, self, _unitName)
                local _SmIm = MENU_GROUP_COMMAND:New(group, "Smoke Impact On/Off", _settingsPath,
                    self._SmokeBombImpactOnOff, self, _unitName)
                local _FlHi = MENU_GROUP_COMMAND:New(group, "Flare Hits On/Off", _settingsPath,
                    self._FlareDirectHitsOnOff, self, _unitName)
                local _AlMeA = MENU_GROUP_COMMAND:New(group, "All Messages On/Off", _settingsPath,
                    self._MessagesToPlayerOnOff, self, _unitName)
                local _TrpSh = MENU_GROUP_COMMAND:New(group, "Targetsheet On/Off", _settingsPath,
                    self._TargetsheetOnOff, self, _unitName)

                -- F10/On the Range/<Range Name>/Range Information
                local _WeIn = MENU_GROUP_COMMAND:New(group, "General Info", _infoPath, self._DisplayRangeInfo, self,
                    _unitName)
                local _WeRe = MENU_GROUP_COMMAND:New(group, "Weather Report", _infoPath, self._DisplayRangeWeather,
                    self, _unitName)
                local _BoTgtgs = MENU_GROUP_COMMAND:New(group, "Bombing Targets", _infoPath, self._DisplayBombTargets,
                    self, _unitName)
                local _StrPits = MENU_GROUP_COMMAND:New(group, "Strafe Pits", _infoPath, self._DisplayStrafePits, self,
                    _unitName):Refresh()
            end
        else
            self:E(self.lid .. "Could not find group or group ID in AddF10Menu() function. Unit name: " .. _unitName or
                       "N/A")
        end
    else
...
function RANGE:_MarkTargetsOnMap(_unitName)
    ...
    for _, _bombtarget in pairs(self.bombingTargets) do
        ...
        if group then
            coord:MarkToGroup(string.format("Bomb target %s:\n%s\n%s", bombtarget.name, coord:ToStringLLDMS(),
                coord:ToStringBULLS(group:GetCoalition())), group)
        else
            ...
            if _target and _target:IsAlive() then
                ...
                if group then
                    ...
                    coord:MarkToGroup(string.format("Strafe target %s:\n%s\n%s", _target:GetName(),
                        coord:ToStringLLDMS(), coord:ToStringBULLS(group:GetCoalition())), group)
                else
...
function RANGE:_IlluminateBombTargets(_unitName)
    ...
    if #bomb > 0 then
        ...
        local c = COORDINATE:New(coord.x, coord.y + math.random(self.illuminationminalt, self.illuminationmaxalt),
            coord.z)
        c:IlluminationBomb()
    end
...
function RANGE:_SmokeBombTargets(unitname)
    ...
    if unitname then
        local unit, playername = self:_GetPlayerUnitAndName(unitname)
        local text = string.format("%s, %s, bombing targets are now marked with %s smoke.", self.rangename, playername,
            self:_smokecolor2text(self.BombSmokeColor))
...
function RANGE:_SmokeStrafeTargets(unitname)
    ...
    if unitname then
        local unit, playername = self:_GetPlayerUnitAndName(unitname)
        local text = string.format("%s, %s, strafing tragets are now marked with %s smoke.", self.rangename, playername,
            self:_smokecolor2text(self.StrafeSmokeColor))
        self:_DisplayMessageToGroup(unit, text, 5)
    end
...
function RANGE:_SmokeStrafeTargetBoxes(unitname)
    ...
    if unitname then
        local unit, playername = self:_GetPlayerUnitAndName(unitname)
        local text = string.format("%s, %s, strafing pit approach boxes are now marked with %s smoke.", self.rangename,
            playername, self:_smokecolor2text(self.StrafePitSmokeColor))
        self:_DisplayMessageToGroup(unit, text, 5)
    end
...
function RANGE:_playersmokecolor(_unitName, color)
    ...
    if _unit and _playername then
        ...
        local text = string.format("%s, %s, your bomb impacts are now smoked in %s.", self.rangename, _playername,
            self:_smokecolor2text(color))
        self:_DisplayMessageToGroup(_unit, text, 5)
    end
...
function RANGE:_playerflarecolor(_unitName, color)
    ...
    if _unit and _playername then
        ...
        local text = string.format("%s, %s, your direct hits are now flared in %s.", self.rangename, _playername,
            self:_flarecolor2text(color))
        self:_DisplayMessageToGroup(_unit, text, 5)
    end

end

function RANGE:_playersmokeduration(_unitName, duration)
    self:F({
        unitname = _unitName,
        duration = duration
    })
    ...
    local _unit, _playername = self:_GetPlayerUnitAndName(_unitName)
    if _unit and _playername then
        self.PlayerSettings[_playername].smokeDuration = duration
        --local text = string.format("%s, %s, your smoke will now last for %d seconds.", self.rangename, _playername,            duration)
        --self:_DisplayMessageToGroup(_unit, text, 5)
    end
end
